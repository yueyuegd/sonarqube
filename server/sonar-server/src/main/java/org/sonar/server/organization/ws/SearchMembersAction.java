...
public class SearchMembersAction implements OrganizationsWsAction {
  ...
  public SearchMembersAction(DbClient dbClient, UserIndex userIndex, DefaultOrganizationProvider organizationProvider) {
  ...
  public void handle(Request request, Response response) throws Exception {
    try (DbSession dbSession = dbClient.openSession(false)) {
      ...
      List<String> memberLogins = dbClient.organizationMemberDao().selectLogins(dbSession, organization.getUuid());

      UserQuery.Builder userQuery = UserQuery.builder();
      userQuery.setTextQuery(request.param(Param.TEXT_QUERY));
      ...
      SearchMembersWsResponse wsResponse = buildResponse(users, wsPaging);

      writeProtobuf(wsResponse, request, response);
    }
  }

  private static SearchMembersWsResponse buildResponse(List<UserDto> users, Common.Paging wsPaging) {
